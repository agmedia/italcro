...
class ControllerExtensionCaptchaGoogle extends Controller {
	...
	public function index() {
		...
		if (isset($this->request->post['captcha_google_status'])) {
			...
			if (isset($this->request->post['captcha_google_score'])) {
				$data['captcha_google_score'] = $this->request->post['captcha_google_score'];
			} elseif ($this->config->get('captcha_google_score')) {
				$data['captcha_google_score'] = $this->config->get('captcha_google_score');
			} else {
				$data['captcha_google_score'] = '0.5';
			}
