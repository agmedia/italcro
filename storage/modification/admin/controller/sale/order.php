...
class ControllerSaleOrder extends Controller {
	...
	public function getForm() {
		...
		$custom_fields = $this->model_customer_custom_field->getCustomFields(array('filter_customer_group_id' => isset($order_info['customer_group_id']) ? $order_info['customer_group_id'] : '', 'sort' => 'cf.sort_order'));
					...
					if ( !empty($product['product_id']) ) {
						...
						if ( $this->liveopencart_ext_poip->installed() ) {
							if (isset($product['img'])) {
								$product['img'] = array('image'=>$this->liveopencart_ext_poip->getModel()->getProductOrderImage($product['product_id'], $options, (isset($product['img']['image'])) ? $product['img']['image'] : $product['img'] ));
							}
							...
							if ( isset($image) && $this->config->get($this->config->get('config_theme') . '_image_cart_width') && $this->config->get($this->config->get('config_theme') . '_image_cart_height') ) {
								$poip_image = $this->liveopencart_ext_poip->getModel()->getProductOrderImage($product['product_id'], $options, '' );
								if ($poip_image) {
