...
class ModelExtensionModuleQiqo extends Model
{
    ...
    public function updatePrices(): int
    {
        ...
        foreach ($articles as $a) {
            ...
            $cent  = trim((string)($a['cent'] ?? ''));

            // Ako ERP šalje "C-100", cijenu dijelimo sa 100
            if (strtoupper($cent) === 'C-100') {
                $price = $price / 100;
            }
    ...
    {
        $qiqo = new \Agmedia\Api\Connection\Soap\Qiqo();
        $articles = $qiqo->getArticles(); // svaki ima id (sku) i kataloggrupa
        $relatedCount = 0;

        $this->log('Related', '=== START LINK RELATED PRODUCTS (by kataloggrupa) ===');
        $this->log('Related', 'Ukupno artikala iz ERP-a: ' . count($articles));

        // 1️⃣ Grupiraj artikle po kataloggrupa
        $groups = [];
        foreach ($articles as $a) {
            $sku = trim((string)($a['id'] ?? ''));
        ...
        $this->log('Related', 'Generirano ERP grupa: ' . count($groups));

        // 2️⃣ Obradi svaku grupu (ako ima više od 1 artikla)
        foreach ($groups as $gid => $skus) {
            if (count($skus) < 2) continue;

            // pronađi product_id-eve po SKU
            $escaped = array_map([$this->db, 'escape'], $skus);
            $in = "'" . implode("','", $escaped) . "'";
            $query = $this->db->query("SELECT product_id, sku FROM " . DB_PREFIX . "product WHERE sku IN ({$in})");

            $product_ids = array_column($query->rows, 'product_id');
            if (count($product_ids) < 2) continue;

            // 3️⃣ Poveži sve međusobno
            foreach ($product_ids as $pid) {
                foreach ($product_ids as $rid) {
                    if ($pid == $rid) continue;

                    // provjeri postoji li već
                    $exists = $this->db->query("SELECT 1 FROM " . DB_PREFIX . "product_related 
            ...
            $this->log('Related', "✅ ERP grupa {$gid} | SKU count: " . count($product_ids) . " | dodano veza: {$relatedCount}");
        }

        $this->log('Related', "=== END LINK RELATED (by kataloggrupa) | ukupno {$relatedCount} poveznica ===");
        return $relatedCount;
    }
