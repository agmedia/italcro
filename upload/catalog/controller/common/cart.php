...
class ControllerCommonCart extends Controller {
	public function index() {
		...
		$data['text_items'] = sprintf($this->language->get('text_items'), count($this->cart->getProducts()) + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->currency->format($total, $this->session->data['currency']));
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				...
				$unit_price = $this->tax->calculate($display_unit_price, $product['tax_class_id'], $this->config->get('config_tax'));
