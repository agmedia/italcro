	...
	public function add() {
		...
		if ($product_info) {
			...
			if (!$json) {
				...
				$json['total'] = sprintf($this->language->get('text_items'), count($this->cart->getProducts()) + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->currency->format($total, $this->session->data['currency']));
			} else {
