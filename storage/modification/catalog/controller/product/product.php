	...
	if (isset($this->session->data['error'])) {
		...
		$data['thumb_sm'] = $this->model_tool_image->resize($product_info['image'], $this->config->get('theme_default_image_additional_width'), $this->config->get('theme_default_image_additional_height'));
			...
			$results = $this->model_catalog_product->getProductImages($this->request->get['product_id']);
				...
				if ( class_exists('\liveopencart\ext\poip') ) {
					...
					$this->liveopencart_ext_poip->getModel()->addProductPageResources();

					if ( $this->config->get('module_digitcart_image_resizer_status') ) { // comp with digitcart_image_resizer
						$data['poip_img_sizes'] = [];
						if ($additional_width && $additional_height) {
							$data['poip_img_sizes']['thumb_width'] = $additional_width;
							$data['poip_img_sizes']['thumb_height'] = $additional_height;
						}
					}
					
					$poip_data = $this->liveopencart_ext_poip->getModel()->getDataForProductPage(isset($this->request->get['product_id']) ? $this->request->get['product_id'] : false, $results, $data);
					if ( $poip_data ) {
		...
		'thumb_lg' => $this->model_tool_image->resize($result['image'], $this->config->get('theme_default_image_thumb_width'), $this->config->get('theme_default_image_thumb_height')),
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				...
				// << Product Option Image PRO module
				'poip_image' => class_exists('\liveopencart\ext\poip') ? $this->liveopencart_ext_poip->getModel()->getProductOptionValuePOIPThumbResized($option_value) : [],
				// >> Product Option Image PRO module
		...
		if (strtotime($result['date_available']) > strtotime('-' . $this->config->get('newlabel_status') . ' day')) {
			...
			$is_new = false;
				...
				// << Product Option Image PRO module
				'option_images'  => class_exists('\liveopencart\ext\poip') ? $this->liveopencart_ext_poip->getModel()->getCategoryImagesForController( $result['product_id'], "related_products" ) : [],
				// >> Product Option Image PRO module
...
if(file_exists('catalog/model/extension/cmpltguagaf.php')) { 
				...
				$data['footer'] .= $this->model_extension_cmpltguagaf->viewcont(isset($this->request->get['product_id']) ? $this->request->get['product_id'] : 0);
				   ...
				   if (version_compare(VERSION, '2.3') >= 0) {
					$data['attachfile'] = $this->load->controller('extension/module/mmos_attachmanager/attachfile', $product_info);
				 } else {
				 	$data['attachfile'] = $this->load->controller('module/mmos_attachmanager/attachfile', $product_info);
				 }
			...
			///xml sep 2019 ///
				...
				$data['faqcategorypro'] = $this->load->controller('extension/faqcategorypro');
			/// xml Work sep 2019 ///
