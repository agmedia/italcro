...
class ControllerExtensionBaselQuickview extends Controller {
	public function index() {
		...
		if ($product_info) {
			...
			$data['poip_img_sizes'] = array(
				'popup_width' => $this->config->get('quickview_popup_image_width'),
				'popup_height' => $this->config->get('quickview_popup_image_height'),
			);
			
			$poip_data = $this->liveopencart_poip->getModel()->getDataForProductPage($this->request->get['product_id'], $results, $data);
			// quickview do not load scripts normally
			$data['poip_scripts'] = array_merge($this->liveopencart_poip->getModel()->getHeaderScripts(), $this->liveopencart_poip->getModel()->getProductPageScripts() );
			if ( $poip_data ) {
